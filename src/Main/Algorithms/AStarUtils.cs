...
using USC.GISResearchLab.ShortestPath.Precomputation;
...
namespace USC.GISResearchLab.Routing.Algorithms
{
	...
	{
		public static int MIN_NearestLevelCount = 6;
		...
		public static NearestNodeIO FindNearestNode(NearestNodeIO io)
		{
			try
			{
                ...
                double te1 = 0.0, te2 = 0.0;
                double alpha = 0.0;
                double sqlDist = 0.0, p2inp = 0.0;
                ...
                for (buffer = initBuffer; (io.NearestNodes.Count < MIN_NearestLevelCount) && (buffer <= maxBuffer); buffer = buffer * 2)
                {
					...
					while (read.Read())
					{
						...
						midNode1 = new Node(new GraphNode(midpoint.Long.Value, midpoint.Lat.Value), false);
						midNode2 = new Node(new GraphNode(midpoint.Long.Value, midpoint.Lat.Value), false);
						circle = null;

                        // I have no idea what I've done before to calculate walking
                        ...
                        for (int i = 2; i <= count; i++)
                        {
                            ...
                            if (te1 * te2 >= 0.0)
                            {
                                ...
                                if ((alpha >= 0) && (alpha <= 1) && (l2 < dist))
                                {
                                    ...
                                    // midNode1 = new Node(new GraphNode((shape.STPointN(i - 1).Long.Value + shape.STPointN(i).Long.Value) / 2, (shape.STPointN(i - 1).Lat.Value + shape.STPointN(i).Lat.Value) / 2));
                                    // midNode2 = new Node(new GraphNode((shape.STPointN(i - 1).Long.Value + shape.STPointN(i).Long.Value) / 2, (shape.STPointN(i - 1).Lat.Value + shape.STPointN(i).Lat.Value) / 2));
                                }
                            }
                            travel += p1p2;
                        }
                        ...
                        privateRoad = read.GetString(12) != "N";
