...
namespace USC.GISResearchLab.Routing.Algorithms
{
	public class AStar
	{
		...
		BinaryHeap openList;
		Dictionary<long, Node> closedList;
		Graph myGraph;
		// public Hashtable precomputes;
		// public List<Node> landMarkList;
		// Node bestLM;
		...
		*/
		...
		public AStarOutput CalculatePath(ref AStarInput input)
		{
			...
			byte i = 0;
            bool isHeapContainsN = false, SearchIsFinished = false, UnpurifiedSearchOfPrivateArea = false;
			...
			try
			{
				...
				try
				{
					...
					openList = new BinaryHeap(1000);
					closedList = new Dictionary<long, Node>(10000);
					...
					foreach (var l in input.From.NearestNodes)
					{
						...
						foreach (var n in l.Value)
						{
							current = new Node(myGraph.GetNode(n.UID), n.PureBloodPrivate);
							...
							if (!openList.Contains(current) && !closedList.ContainsKey(current.UID))
							{
								openList.Insert(current);
								if (FromNodesPrev.ContainsKey(current.UID))
									FromNodesPrev[current.UID] = n.PreviousNode.PreviousNode.UID;
								else FromNodesPrev.Add(current.UID, n.PreviousNode.PreviousNode.UID);
							}
						}
						...
						while (!openList.isEmpty())
						{
							...
							if (closedList.ContainsKey(current.UID)) throw new Exception("Closed/Open List violation.");
							closedList.Add(current.UID, current);
							...
							if (ToNN.ContainsKey(current.UID))
							{
								...
								DiscoveredDestinations.Add(current, ToNN[current.UID]);
                            ...
                            // if (current.PreviousNode != input.From.InpNode)
                            //     UnpurifiedSearchOfPrivateArea = current.PreviousNode.Neighbors[current.PreviousEdge].PrivateRoad && !current.PureBloodPrivate;
                            // else UnpurifiedSearchOfPrivateArea = false;

							for (i = 0; i < current.NeighborsCount; i++)
							{
								gNeighbor = myGraph.GetNode(current.Neighbors[i].DestinationNodeID);
								// if already expanded, skip it
								if (closedList.ContainsKey(gNeighbor.UID)) continue;

                                // Only a pure blood can exit a private section
                                // if (!current.Neighbors[i].PrivateRoad && UnpurifiedSearchOfPrivateArea) continue;

								// Manoeuvre Restriction. Do restriction test only when it's not the source node.            
                                if (current.PreviousNode != input.From.InpNode)
                                {
                                    if (current.Neighbors[i].ContainsRestriction(current.PreviousNode.UID))
                                    {
                                        ret.ManoeuvreCount++;
                                        continue;
                                    }
                                ...
                                {
                                    if (current.Neighbors[i].ContainsRestriction(FromNodesPrev[current.UID]))
                                    {
                                        ret.ManoeuvreCount++;
                                        continue;
                                    }
                                ...
                                neighbor = openList.FindGraphNode(gNeighbor, false);
								isHeapContainsN = openList.Contains(neighbor);
								...
								if (!isHeapContainsN)
								{
                                    ...
                                    // neighbor.PureBloodPrivate = current.PureBloodPrivate && current.Neighbors[i].PrivateRoad;
									neighbor.H = Heuristic(neighbor, input.To.InpNode, input.Metric);
									// Don't go too Far
									if (neighbor.F < UpperF) openList.Insert(neighbor);
								}
								else if (isHeapContainsN && (alternativeG < neighbor.G))
								{
									openList.DecreaseKey(neighbor, alternativeG);
                                    neighbor.PreviousNode = current;
                                    neighbor.PreviousEdge = i;

                                    /* There is a big assumption here:
                                     * Consider the scenarion where an unpure node is decreasing g-value of a pure blood
                                     * node and hence it's also unpurifying it as well. Now the unpurified node cannot exit
                                     * the private section (before it could). The question is if we had let it keep the
                                     * bigger g-value and stay pure, will it get to the destination faster!?
                                     */
                                    // neighbor.PureBloodPrivate = current.PureBloodPrivate && current.Neighbors[i].PrivateRoad;
								}
							}
			...
			{
				if (openList != null) ret.OpenNodesCount = openList.Count;
